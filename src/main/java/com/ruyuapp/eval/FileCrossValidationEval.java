...
import com.ruyuapp.counter.FileCounter;
import com.ruyuapp.utils.FileUtils;
 ...
 */
public class FileCrossValidationEval {
    ...
    public FileCrossValidationEval(TextModel classifierType) {
    ...
    public void eval() {
        ...
        for (int j = 0; j < 10; j++) { //循环10次，1个做测试集，其他9个做训练集
            ...
            tp.clear();
            counter = CounterFactory.getFileTypeInstance(1, 20 * j + 1, 20 * j + 21, -1);
            ...
            for (String category : catagories) {
                try {
                    File[] files = ((FileCounter) counter).listFilesByCatagory(category);
                    for (int i = 20 * j; i < 20 * j + 20; i++) {
                        System.out.println((j + 1) + "折=>" + category + "=>" + (i - 20 * j));
                        String result = classifier.classify(FileUtils.getFileText(files[i]));
                        if (result.equals(category)) {
                            ...
                            table.put(category, files[i].getName(), result);
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
